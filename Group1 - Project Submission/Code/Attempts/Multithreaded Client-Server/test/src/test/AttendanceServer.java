...
public class AttendanceServer {
    ...
    public void run() {
    	...
    	byte[] ba1 = new byte[255];
    	byte[] ba2 = new byte[255];
    	byte[] ba3 = new byte[255];
    	...
    	try {
    		...
    		datain.read(ba1,0,255);
    		current = new String(ba1);
	        ...
	        //System.out.println("writing " + Classes.length() + " bytes");
	
	        //Writing the classes found to the client (351)
	        dataout.write(Classes.getBytes(),0,Classes.length());
	        dataout.write("\n".getBytes(),0,1);
	        dataout.flush();
    	    
	        //Reading class chosen by the client
	        datain.read(ba2, 0, 255);
    		current2 = new String(ba2);
    		String table = OpenClass(current2);
    		
    		//Writing class information to the client
    		dataout.write(table.getBytes(),0, table.length());
            dataout.write("\n".getBytes(),0,1);
   	        dataout.flush();

   	        while(conversationActive) {
   	   	        datain.read(ba3,0,255);
   	   	        request = new String(ba3);
	    	    if(request.contains("exit")){
	        		System.out.println("Exiting");
   	    	        ...
   	    	        dataout.write(table2.getBytes(),0, table2.length());
   	    	        dataout.write("\n".getBytes(),0,1);
   	    	        dataout.flush();
   	   	        }
   	   	        else if(request.charAt(0)=='2') {
   	   	        	current3 = new String(ba3);
   	   	        	String table2 = Order(current3.substring(1)); 
   	   	        	dataout.write(table2.getBytes(),0, table2.length());
	    	        dataout.write("\n".getBytes(),0,1);
	    	        dataout.flush();
   	   	        }
    ...
    private String Order(String var) {
    	String result = "None available";
        Connection conn = null;
        try {
            conn = DriverManager.getConnection(dbURL2,"root","Sah82@MySQL");
            Statement stmt = conn.createStatement();
            String query = "SELECT * " + "FROM StudentsAttendance " + " ORDER BY out_time" ;
            System.out.println("query = " + query);
            ResultSet rs = stmt.executeQuery(query);
            ResultSetMetaData rsmd = rs.getMetaData();
            int nbCols = rsmd.getColumnCount();
